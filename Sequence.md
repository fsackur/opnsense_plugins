Hi Ad, I can now see with my own eyes the complexity you pointed out for a complete solution.

I imagine how you would view the project and I understand that it is of marginal benefit to you. I aim to make it easy for you to say "no" as quickly as possible. (Worst case is using your time to eventually say no.)

I include the most alarming code snippets below. I don't think I can do _much_ better on those bits - so if you don't like them, you have a great opportunity to say "no" now.

I don't suggest you look at it, but just for reference, I've pushed an alpha at https://github.com/fsackur/opnsense_plugins/pull/4.

## Edge cases

I have found some edge cases in detecting POST and request bodies. Fortunately, the coding convention throughout makes it easy to sidestep those. Risk still exists. See code snippets in [ParseControllers.php](#parsecontrollersphp).

I currently see no edge cases in the XML parsing. But I cannot have high confidence until I write a test suite. See code snippets in [generate\_openapi\_spec.py](#generate_openapi_specpy).
...
- model property constraints and qualifiers (e.g. Required, Mask, Default)
  - mostly, won't do
  - will do Required and some others if low effort
- primitive types
  - currently, all string. Since non-body params are in the path, this is not a big deal
  - will do, low effort
- optional path params
  ...
  - won't do NetworkinsightController, lol
...
<details><summary><h3>Output: models.json</h3></summary>
...
<details><summary><h3>Code: looking for POST requests</h3></summary>
...
<details><summary><h3>Code: looking for paths that relate models to request bodies</h3></summary>
...
<details><summary><h3>Output: controllers.json</h3></summary>
...
Processes PHP doc comments, transforms backslashes and normalises model paths (`model_path_map`). `model_path_map` is a tuple of the args passed to setBase/addBase/etc.

Generates the OpenApi `path` and `operationId`. These are properties, so they are not seen in the json.

<details><summary><h3>Output: endpoints.json</h3></summary>
...
<details><summary><h3>Code: get_model_spec</h3></summary>
...
There is one more slightly tricky method: `resolve_component_path`. Many controllers do not accept or return the entire XML model. This behaviour is declared in the controllers by the params in calls to setBase/AddBase/etc, and surfaced as a tuple in `model_path_map`. E.g.:

- `alias:aliases.alias`
- `rule:userDefinedRules.rule`
- `:Connections.Connection`  (in this case, the client does not wrap the model in a property)

The job of `resolve_component_path` is to give us a valid `$ref` for request bodies and responses. E.g.:

- `("alias", "opnsense.firewall.alias/properties/aliases/items/properties/alias")`
- `("rule", "opnsense.ids.ids/properties/userDefinedRules/items/properties/rule")`
- `(None, "opnsense.ipsec.swanctl/properties/Connections/items/properties/Connection")`

We have to pass in the components from `get_model_spec`, so that we can pick our way through the arrays and objects.

<details><summary><h3>Code: resolve_component_path</h3></summary>
...
<details><summary><h3>Output: openapi.yml</h3></summary>
